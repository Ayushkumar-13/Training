      ...
      <div className="max-w-md mx-auto px-4 py-16">
        <div className={`border p-8 rounded-2xl shadow-sm ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
          <div className="text-center mb-6">
            <h1 className="text-2xl font-black">Sign In to MedEquip</h1>
            <p className={`text-xs mt-1 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>Access your healthcare purchasing account.</p>
          </div>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <div className="text-center pt-2">
              <a href="/register" className={`text-xs font-bold transition-colors ${darkMode ? 'text-slate-400 hover:text-cyan-400' : 'text-slate-600 hover:text-cyan-600'}`}>
