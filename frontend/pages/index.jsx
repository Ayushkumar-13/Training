...
export default function Home() {
  ...
  return (
    <main className={`min-h-screen transition-colors duration-200 ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      {/* Top Clinical Announcement Bar - Light Mode Harmonized */}
      <div className={`py-2 px-4 text-center text-xs font-bold transition-colors border-b ${darkMode ? 'bg-cyan-950/80 text-cyan-300 border-cyan-900' : 'bg-cyan-50 text-cyan-900 border-cyan-200'}`}>
        <span>🏥 ISO 13485 Certified • Hospital PO Billing • 24/7 Clinical Support Hotline</span>
      </div>

      {/* Hero Banner */}
      <section className={`py-20 px-4 sm:px-6 lg:px-8 border-b transition-colors duration-200 ${darkMode ? 'bg-slate-900 border-slate-800 text-white' : 'bg-gradient-to-b from-slate-100 via-cyan-50/40 to-slate-50 border-slate-200 text-slate-900'}`}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h1 className={`text-4xl sm:text-5xl font-black tracking-tight leading-tight ${darkMode ? 'text-white' : 'text-slate-900'}`}>
              Hospital-Grade Equipment. <span className={darkMode ? 'text-cyan-400' : 'text-cyan-600'}>Guaranteed Precision.</span>
            </h1>
            ...
            <div className="mt-6 flex flex-wrap gap-2">
              <a href="/products?category_slug=diagnostic-imaging" className={`px-3 py-1 text-xs font-bold rounded-lg border transition-all ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100 shadow-sm'}`}>
                Diagnostic Imaging
              </a>
              <a href="/products?category_slug=patient-monitoring" className={`px-3 py-1 text-xs font-bold rounded-lg border transition-all ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100 shadow-sm'}`}>
                Patient Monitoring
              </a>
              <a href="/products?category_slug=infusion-pumps" className={`px-3 py-1 text-xs font-bold rounded-lg border transition-all ${darkMode ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-100 shadow-sm'}`}>
                Infusion Pumps
              </a>
            </div>
          </div>
          ...
          <div className="relative">
            <div className={`w-full h-80 rounded-2xl border p-6 flex flex-col justify-between shadow-xl relative overflow-hidden group ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-200 border-slate-300'}`}>
              <img
              ...
              <div className={`relative z-10 p-5 rounded-xl border backdrop-blur-sm ${darkMode ? 'bg-slate-900/90 border-slate-800 text-white' : 'bg-white/90 border-slate-200 text-slate-900'}`}>
                <span className="text-xs font-bold text-emerald-500 uppercase tracking-widest">Featured Device</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {categories.slice(0, 5).map((cat, idx) => (
            <a
              ...
              className={`p-6 rounded-2xl border transition-all duration-200 group hover:-translate-y-1 ${darkMode ? 'bg-slate-900 border-slate-800 hover:border-cyan-700' : 'bg-white border-slate-200 hover:border-cyan-500 shadow-sm hover:shadow-md'}`}
            >
